...
int BulletPool::getActive()
{
	return active;
}

void BulletPool::Alloc()
{
	std::uniform_int_distribution<int> dist(0, 2);
	int reservation = (int) (ceil(gos.size() / PREALLOCAMOUNT) + 1) * PREALLOCAMOUNT;
	...
	{
		rects.emplace_back(SDL_Rect{ 0, 0, WIDTH / 2, HEIGHT / 2 });
...
bool BulletPool::Compare(std::tuple<float, float>& a, std::tuple<float, float>& b)
{
...
void BulletPool::InsertionSort(int index, int l, int r)
{
	SDL_Rect rtemp;
...
/*void BulletPool::Merge(int startIndex, std::vector<std::tuple<float, float>>& aux, int auxL, int startAuxL, int sizeAuxL, int auxR, int startAuxR, int sizeAuxR)
{
...
void BulletPool::MergeSortNoCopy(std::vector<std::tuple<float, float>>& aux, int left, int right)
{
...
int BulletPool::InitialDirection(int index, int x, int y)
{
	dirs[index] = { x, y };
	return index;
}

int BulletPool::Activate(unsigned int amount)
{
...
void BulletPool::Deactivate(unsigned int index)
{
	deactivation.push_back(index);
}

void BulletPool::Deactivate()
{
	std::sort(deactivation.begin(), deactivation.end(), std::greater<int>());
	for (unsigned int& each : deactivation)
	{
		if (each > active)
			continue;
...
void BulletPool::Update(float deltaTime, int WINWIDTH, int WINHEIGHT)
{
	...
	std::cout << "BulletPool active: " << active << std::endl;
}

std::vector<SDL_Rect>* BulletPool::Physics()
{
...
void BulletPool::Render(SDL_Renderer* render)
{
